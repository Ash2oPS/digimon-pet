...
The source URLs are official Digimon Web JPGs. The app stores local PNG copies with the edge-connected white background made transparent. The stats window uses the local transparent artwork when present, falls back to the runtime sprite otherwise, and fetches missing artwork in the background.
